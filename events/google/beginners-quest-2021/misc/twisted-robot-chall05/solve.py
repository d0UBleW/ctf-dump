...
"""
https://github.com/pberba/ctf-solutions/tree/master/20180907_nox/plot_twist
...
num = [ (int(''.join(i.split('-'))) - (1<<31)) for i in num ]

rand = mtb.go(num)

key = []
for i in range(31):
    key.append(int(rand.getrandbits(8)))

with open("secret.enc", "rb") as f:
    secret = f.read()

for i in range(31):
    print(chr(secret[i] ^ key[i]), end='')
